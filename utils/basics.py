...
def train(generator, target_model, dataloader, device = 'cpu', num_epochs=5, z_dim=100, lr=1e-3, verbose=True):
    ...
    for epoch in range(num_epochs):
        ...
        return train_losses
def gen_img_plot1(test_input, epoch):
    prediction = np.squeeze(test_input)
    ...
    plt.axis('off')
    plt.savefig("images/dynet" + str(i) + str("_") + str(epoch + 1) + ".png")
    plt.clf()
def gen_img_plot(model, test_input, epoch, round,num_common_layer):
    ...
    if not os.path.isdir(results_dir_1):
        os.makedirs(results_dir_1)
    plt.axis('off')
    plt.savefig("images/dynet" + str(round) + str("_") + str(epoch + 1)+ str("_") + str(num_common_layer) + ".png")
    plt.clf()


def generic_train(model, rounds, num_epochs,  trainloader, optimizer, criterion, attack, device="cpu", verbose=False):
    """
    train a model
    Args:
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple time
        ...
        for i, data in enumerate(trainloader, 0):
            inputs, labels = data[0].to(device), data[1].to(
                device)  # get the inputs; data is a list of [inputs, labels]
...
def GAN_train(model, round, num_epochs, num_common_layer,trainloader, optimizer, g_optimizer, criterion, attack, device="cpu", verbose=False):
    """
    train a model
    Args:
        model (torch.nn.Module): the model to train
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple time
        ...
        for i, data in enumerate(trainloader, 0):
            inputs, labels = data[0].to(device), data[1].to(
                device)  # get the inputs; data is a list of [inputs, labels]
            # print(inputs.shape)
            labels1 = torch.full_like(data[1], 3)
            labels2 = torch.randint_like(data[1], 10)
            ...
            #print(labels)
            labels1.to(device)
            z = torch.rand((labels.size()[0], 100))
            ...
            loss.backward()
            d_loss = loss
            # gan forward + backward + optimize
            gen_img = generator(z)
            gen_img = gen_img.unsqueeze(1)
            if np.random.uniform(0, 1) < epsilon:
                fake_output = model(gen_img.detach())
                d_fake_loss = criterion(fake_output, labels2)
                d_fake_loss.backward()
                if verbose:
                    d_running_loss += d_fake_loss.item()
                d_loss = d_fake_loss + loss
            optimizer.step()

            epoch_loss += loss.item()
            g_optimizer.zero_grad()
            fake_output = model(gen_img)
            g_loss = criterion(fake_output, labels1)
            g_loss.backward()
            g_optimizer.step()
            if verbose:
                running_loss += loss.item()
                ...
                if i % print_every == 0 and i != 0:
                    # print("GAN_epsilon:", epsilon)
                    # print(f"[epoch: {epoch}, datapoint: {i}] \t loss: {round(running_loss / print_every, 3)}")
                    # print(f"[epoch: {epoch}, datapoint: {i}] \t d_loss: {round(d_running_loss / print_every, 3)}")
                    # print(f"[epoch: {epoch}, datapoint: {i}] \t g_loss: {round(g_running_loss / print_every, 3)}")
                    running_loss = 0.0
                    d_running_loss = 0.0
                    g_running_loss = 0.0
            with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            if epoch % 2== 10:
                ...
                # print(D_loss)
                # print(G_loss)
                gen_img_plot(generator, z, epoch, round, num_common_layer)
